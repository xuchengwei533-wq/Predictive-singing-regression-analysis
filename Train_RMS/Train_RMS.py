...
output_dir = r"D:\同济车辆\chest_audio\Train_RMS"
...
def build_dataset():
    if not os.path.isdir(rms_dir):
        raise FileNotFoundError(f"特征目录不存在: {rms_dir}")
    files = [f for f in os.listdir(rms_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if rms_stats is None:
            continue
        feat = np.array([rms_stats[0], rms_stats[1]], dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)
