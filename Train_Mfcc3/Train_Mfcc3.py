...
output_dir = r"D:\同济车辆\chest_audio\Train_Mfcc3"
...
def build_dataset():
    if not os.path.isdir(mfcc3_dir):
        raise FileNotFoundError(f"特征目录不存在: {mfcc3_dir}")
    files = [f for f in os.listdir(mfcc3_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if label is None:
            continue
        mfcc_path = os.path.join(mfcc3_dir, base + ".csv")
        if not os.path.exists(mfcc_path):
            continue
        mfcc_stats = stats_mean_std(load_series_csv(mfcc_path))
        if mfcc_stats is None:
            continue
        feat = np.array(mfcc_stats, dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)
