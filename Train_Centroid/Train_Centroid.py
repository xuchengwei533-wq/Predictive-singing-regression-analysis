...
def build_dataset():
    if not os.path.isdir(centroid_dir):
        raise FileNotFoundError(f"特征目录不存在: {centroid_dir}")
    files = [f for f in os.listdir(centroid_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if centroid_stats is None:
            continue
        feat = np.array([centroid_stats[0], centroid_stats[1]], dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)
