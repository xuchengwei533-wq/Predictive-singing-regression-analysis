...
output_dir = r"D:\同济车辆\chest_audio\Train_F0Pyin"
os.makedirs(output_dir, exist_ok=True)

f0_dir = os.path.join(data_dir, "F0_PYIN_Output")
...
def build_dataset():
    if not os.path.isdir(f0_dir):
        raise FileNotFoundError(f"特征目录不存在: {f0_dir}")
    files = [f for f in os.listdir(f0_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if label is None:
            continue
        f0_path = os.path.join(f0_dir, base + ".csv")
        if not os.path.exists(f0_path):
            continue
        f0_stats = stats_mean_std(load_series_csv(f0_path))
        if f0_stats is None:
            continue
        feat = np.array([f0_stats[0], f0_stats[1]], dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)
